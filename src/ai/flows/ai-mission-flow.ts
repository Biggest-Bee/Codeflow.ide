...
export async function processAiMissionInternal(input: AiMissionInput): Promise<AiMissionOutput> {
  try {
    ...
    // Using string concatenation to avoid complex backtick nesting issues in template literals
    const workspaceContextText = (input.workspaceContext || []).map((node: any) => {
      const header = "### " + node.type + ": " + node.path + "\n";
    ...
    const { output } = await genkitInstance.generate({
      ...
      prompt: `You are an Autonomous AI Architect using Gemini 2.5 Flash.
