...
export const Editor: React.FC = () => {
  ...
  useEffect(() => {
    if (activeFileId !== lastSyncedId.current) {
      ...
      if (activeFile) {
        addLog(`Opened file: ${activeFile.name}`, 'info');
      }
    }
  ...
  return (
    <div className="flex-1 flex flex-col bg-background h-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div 
          className={cn(
            "border-t bg-sidebar/95 backdrop-blur-md transition-all duration-300 ease-in-out shrink-0",
            isTerminalOpen ? "h-40" : "h-7"
          )}
        >
          <div 
            className="h-7 px-4 flex items-center justify-between cursor-pointer hover:bg-white/5"
            onClick={() => setIsTerminalOpen(!isTerminalOpen)}
          >
            <div className="flex items-center gap-2">
              <TerminalIcon size={12} className="text-primary" />
              <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Output Console</span>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="icon" className="h-5 w-5 hover:text-destructive" onClick={(e) => { e.stopPropagation(); setTerminalLogs([]); }}>
                <Trash2 size={10} />
              </Button>
          ...
          {isTerminalOpen && (
            <ScrollArea className="h-[calc(100%-1.75rem)] p-2">
              <div className="space-y-1 font-code text-[10px] px-2">
                {terminalLogs.length === 0 ? (
                  <div className="opacity-20 uppercase font-black py-4">Waiting for process signals...</div>
                ) : (
                  terminalLogs.map((log, i) => (
                    <div key={i} className="flex gap-2">
                      <span className="text-muted-foreground/30 shrink-0">[{new Date().toLocaleTimeString([], { hour12: false })}]</span>
                      <span className={cn(
      ...
      <div className="h-6 border-t px-4 flex items-center justify-between bg-sidebar/50 text-[9px] text-muted-foreground/50 font-bold uppercase tracking-[0.15em] shrink-0">
        <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-accent flex items-center gap-1"><Scale size={10} className="text-accent" /> Apache 2.0</span>
        </div>
      </div>
    </div>
  );
