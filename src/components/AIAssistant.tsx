...
import { AiCodeExplanationAndDebuggingOutput } from '@/ai/schemas';
...
export const AIAssistant: React.FC = () => {
  ...
  const getWorkspaceContext = (wsId: string) => {
    ...
    return relevantNodes.map(n => ({
      ...
      type: n.type,
      content: n.content?.slice(0, 10000)
    }));
  };

  const handleFetchAi = async (endpoint: string, payload: any) => {
    ...
    try {
      ...
      throw new Error(`Cloud handshake failed: ${text.slice(0, 100) || "Empty Response"}`);
    }

    if (!res.ok || result.error) {
      throw new Error(result.error || `Synthesis Failed (${res.status})`);
    }
    return result;
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      const workspaceContext = getWorkspaceContext(targetWsId);
      const result = await handleFetchAi('/api/ai/generate', {
      ...
      if (result.operations && result.operations.length > 0) {
        ...
        const firstFileIdx = result.operations.findIndex((o: any) => o.type !== 'createFolder');
        setActiveReviewIdx(firstFileIdx >= 0 ? firstFileIdx : 0);
        toast({ title: "Build Ready", description: "Review the generated module." });
      } else if (result.explanation && result.explanation.includes("Internal Crash:")) {
        toast({ title: "Synthesis Failed", description: result.explanation, variant: "destructive" });
      }
    } catch (e: any) {
      console.error("Full AI Execution Error:", e);
      let errorMessage = e.message || "An unexpected error occurred.";
      if (errorMessage.includes("parse") || errorMessage.includes("validation")) {
        errorMessage = "The AI generated too much code. Try requesting fewer files.";
      }
      toast({ title: "Synthesis Failed", description: errorMessage, variant: "destructive" });
  ...
  const handleDebug = async (isDeepScan: boolean = false) => {
    ...
    try {
      const files = Object.values(nodes)
        ...
        .map(n => ({ fileName: getNodePath(n.id), fileContent: n.content?.slice(0, 15000) || '' }));

      const result = await handleFetchAi('/api/ai/debug', {
      ...
      if (result.operations && result.operations.length > 0) {
        ...
        const firstFileIdx = result.operations.findIndex((o: any) => o.type !== 'createFolder');
        setActiveReviewIdx(firstFileIdx >= 0 ? firstFileIdx : 0);
        toast({ title: "Edit Complete", description: "Refinement patches available." });
      }
    } catch (e: any) {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const missionDescription = autoSelectDeps 
        ? `${editPrompt}\n\nAUTO-DEPENDENCY DETECTION ENABLED: Identify missing modules.`
        : editPrompt;

      const result = await handleFetchAi('/api/ai/mission', {
        missionTitle: "Workspace Recovery",
        missionDescription,
        apiKey: getSelectedKey()!,
        workspaceContext
      });
      ...
      if (result.operations && result.operations.length > 0) {
        ...
        const firstFileIdx = result.operations.findIndex((o: any) => o.type !== 'createFolder');
        setActiveReviewIdx(firstFileIdx >= 0 ? firstFileIdx : 0);
        toast({ title: "Recovery Ready" });
      }
    } catch (e: any) {
      console.error("AI Mission Error:", e);
      toast({ title: "Recovery Error", description: e.message, variant: "destructive" });
  ...
  const handleRenameTeam = async () => {
    if (!activeTeamId || !renamingTeamValue.trim()) return;
    await renameTeam(activeTeamId, renamingTeamValue.trim());
  ...
  const handleInstallDependency = async (pkg: Dependency, workspaceIds: string[]) => {
    for (const wsId of workspaceIds) {
      const pkgJsonNode = Object.values(nodes).find(
        n => n.workspaceId === wsId && n.name === 'package.json' && n.type === 'file'
      );

      if (pkgJsonNode) {
        try {
          const content = JSON.parse(pkgJsonNode.content || '{}');
          if (!content.dependencies) content.dependencies = {};
          content.dependencies[pkg.name] = pkg.type === 'npm' ? 'latest' : '1.0.0';
          updateNode(pkgJsonNode.id, { content: JSON.stringify(content, null, 2) });
          toast({ title: "Module Routed", description: `${pkg.name} added to workspace.` });
        } catch (e) {
          toast({ title: "Install Failed", description: "Malformed package.json", variant: "destructive" });
        }
      } else {
        toast({ title: "Target Missing", description: "No package.json found in workspace.", variant: "destructive" });
      }
    }
  };
  ...
  const handleLeaveTeam = async () => {
    ...
    if (confirm("Are you sure you want to leave this team? You will lose access to shared workspaces.")) {
      await kickMember(activeTeamId, user.id);
    }
  };

  const handleDisbandTeam = async () => {
    if (!activeTeamId) return;
    if (confirm("DISBAND TEAM? All shared assets will be recovered to the owners. No member list will remain.")) {
      ...
      parts.forEach((part: string, i: number) => {
        ...
        if (!pathMap[currentPath]) {
          const newNode: ReviewNode = {
            ...
            children: [],
            opIndex: i === parts.length - 1 ? index : undefined
          };
    ...
    <div className="flex flex-col h-full w-full bg-sidebar/30 overflow-hidden relative">
      <div className="p-1.5 border-b bg-sidebar/50 shrink-0">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className="text-[6px] h-3.5 border-primary/30 bg-primary/10 text-primary uppercase font-black px-1.5">v2.0</Badge>
        </div>
      </div>
            ...
            <TabsContent value="generate" className="mt-0 space-y-3 relative min-h-[400px]">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-2 rounded bg-primary/5 border border-primary/10">
                  ...
                  <Switch checked={autoSelectDeps} onCheckedChange={(val) => setAutoSelectDeps(val)} className="scale-75 origin-right" disabled={isGenerating} />
                </div>
            ...
            <TabsContent value="edit" className="mt-0 space-y-3 relative min-h-[400px]">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-2 rounded bg-primary/5 border border-primary/10">
                  ...
                  <Switch checked={autoSelectDeps} onCheckedChange={(val) => setAutoSelectDeps(val)} className="scale-75 origin-right" disabled={isEditing} />
                </div>
                ...
                <div className="space-y-4">
                  {user && !user.isAnonymous ? (
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-1">
                            <Select value={activeTeamId || ''} onValueChange={setActiveTeam}>
                              <SelectTrigger className="h-7 text-[9px] bg-background/30 border-white/5 flex-1">
                            ...
                            {isTeamOwner && activeTeamId && (
                              <Button 
                                ...
                                onClick={() => {
                                  ...
                                  if (team) {
                                    setRenamingTeamValue(team.name);
                          ...
                          {isRenamingTeam && (
                            <div className="flex items-center gap-1 px-1 animate-in fade-in slide-in-from-top-1">
                              ...
                              />
                              <Button variant="ghost" size="icon" className="h-7 w-7 text-primary" onClick={handleRenameTeam}><Check size={14} /></Button>
                              <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => setIsRenamingTeam(false)}><X size={14} /></Button>
                            </div>
                          )}
                                ...
                                {isTeamOwner ? (
                                  <>
                                    ...
                                    <Button variant="outline" className="h-7 text-[8px] font-black uppercase gap-1.5 border-destructive/30 text-destructive hover:bg-destructive/10" onClick={handleDisbandTeam}>
                                      <Trash2 size={10} /> Disband Team
                                    </Button>
                                  </>
                                ) : (
                                  <Button variant="outline" className="h-7 text-[8px] font-black uppercase gap-1.5 border-destructive/30 text-destructive hover:bg-destructive/10" onClick={handleLeaveTeam}>
                    ...
                    <div className="text-center py-4 bg-background/20 rounded border border-dashed border-white/10">
                      <p className="text-[8px] text-muted-foreground uppercase font-black tracking-widest leading-relaxed">Login Required</p>
                    </div>
                  )}
                </div>
              </div>
