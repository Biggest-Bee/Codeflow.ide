...
export const Sidebar: React.FC = () => {
  ...
  const canDelete = !activeWs?.teamId && activeWs?.userId === user?.id;

  const toggleFolder = useCallback((id: string) => setExpandedFolders(prev => ({ ...prev, [id]: !prev[id] })), []);

  const handleCreateWorkspace = async () => {
    if (newWsName.trim()) {
      const teamIdToUse = sidebarTab === 'team' ? activeTeamId || undefined : undefined;
      await createWorkspace(newWsName.trim(), teamIdToUse);
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-0.5">
                {workspaces.map(ws => {
                  ...
                  const isOwnerOfThisWs = ws.userId === user?.id;
                  const canDeleteWs = !ws.teamId && isOwnerOfThisWs;
                  const isRenaming = renamingWsId === ws.id;

                  return (
                    <div key={ws.id} onClick={() => !isRenaming && setActiveWorkspace(ws.id)} onDragOver={e => e.preventDefault()} onDrop={e => onDropOnWorkspace(e, ws.id)} className={cn("flex items-center justify-between group px-2 py-1.5 rounded cursor-pointer text-[11px] transition-all", activeWorkspaceId === ws.id ? "bg-primary/10 text-primary border border-primary/20" : "hover:bg-secondary/40 text-muted-foreground")}>
                      <div className="flex flex-col min-w-0 flex-1">
                        {isRenaming ? (
                          <Input 
                            autoFocus 
                            className="h-5 text-[10px] py-0 px-1 bg-background border-primary/30" 
                            value={renamingWsValue}
                            onChange={(e) => setRenamingWsValue(e.target.value)}
                            onBlur={() => { if (renamingWsValue.trim()) renameWorkspace(ws.id, renamingWsValue.trim()); setRenamingWsId(null); }}
                            onKeyDown={(e) => e.key === 'Enter' && setRenamingWsId(null)}
                      ...
                      <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 ml-2">
                        <Edit2 size={10} className="hover:text-primary" onClick={e => { e.stopPropagation(); setRenamingWsId(ws.id); setRenamingWsValue(ws.name); }} />
                        <Download size={10} className="hover:text-primary" onClick={e => { e.stopPropagation(); downloadWorkspace(ws.id); }} />
                        {canDeleteWs && <Trash size={10} className="hover:text-destructive" onClick={e => { e.stopPropagation(); deleteWorkspace(ws.id); }} />}
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <div className="flex items-center justify-between px-1">
                    ...
                    {isTeamOwner && <Button variant="ghost" size="icon" className="h-5 w-5 hover:bg-primary/10 hover:text-primary" onClick={() => setIsCreatingWs(true)}><Plus size={12} /></Button>}
                  </div>
                  
                  {isCreatingWs && (
                    <div className="flex items-center gap-1 px-1 animate-in fade-in slide-in-from-top-1">
                      <Input autoFocus placeholder="Team WS name..." className="h-7 text-[10px] bg-background/40" value={newWsName} onChange={e => setNewWsName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleCreateWorkspace()} />
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-primary" onClick={handleCreateWorkspace}><Check size={14} /></Button>
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => setIsCreatingWs(false)}><X size={14} /></Button>
                    </div>
                  )}
                  ...
                  <div className="space-y-0.5">
                    {workspaces.filter(ws => ws.teamId === activeTeamId).map(ws => {
                      const isOwnerOfThisTeam = teams.find(t => t.id === activeTeamId)?.ownerId === user?.id;
                      const isRenaming = renamingWsId === ws.id;
                      
                      return (
                        <div key={ws.id} onClick={() => !isRenaming && setActiveWorkspace(ws.id)} className={cn("flex items-center justify-between group px-2 py-1.5 rounded cursor-pointer text-[11px]", activeWorkspaceId === ws.id ? "bg-primary/10 text-primary border border-primary/20" : "hover:bg-secondary/40 text-muted-foreground")}>
                          ...
                          <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 ml-2">
                            {isTeamOwner && <Edit2 size={10} className="hover:text-primary" onClick={e => { e.stopPropagation(); setRenamingWsId(ws.id); setRenamingWsValue(ws.name); }} />}
                            <Download size={10} className="hover:text-primary" onClick={e => { e.stopPropagation(); downloadWorkspace(ws.id); }} />
                            {isOwnerOfThisTeam && <Trash size={10} className="hover:text-destructive" onClick={e => { e.stopPropagation(); deleteWorkspace(ws.id); }} />}
                          </div>
                        </div>
                      );
                    })}
            ...
            <div className="pt-1.5 border-t border-white/5 flex flex-col items-center gap-1 text-center">
              <div className="flex items-center justify-center gap-1 opacity-40">
                ...
                <span className="text-[6px] font-black uppercase tracking-widest text-muted-foreground">
                  2026 Frostvale Studio • Apache 2.0
                </span>
              </div>
              <LicenseDialog 
                trigger={
