...
// Gemini Developer API models and Vertex Express Mode models
// depend on an API key. API keys should be stored in Cloud Secret Manager
...
// If you are using Vertex Express Mode (vertexAI with apiKey)
// you can get an API key from the Vertex AI Studio Express Mode setup.
...
const ai = genkit({
  plugins: [
    // Load the GoogleAI provider. You can optionally specify your API key
    // by passing in a config object; if you don't, provider uses the value
    // from the GOOGLE_GENAI_API_KEY environment variable, which is
...
export const menuSuggestion = onCallGenkit({
  ...
  // The following will require the user to have email_verified claim,
  // for example.
